...
type User struct {
	...
	AffRebateCount      int            `json:"aff_rebate_count" gorm:"type:int;default:0;column:aff_rebate_count"`
	HasFirstTopupRebate bool           `json:"has_first_topup_rebate" gorm:"type:bool;column:has_first_topup_rebate"`
...
func (user *User) TransferAffAmountToQuota(amount float64) error {
	quota := common.AmountToQuota(amount)
	if quota <= 0 {
		return fmt.Errorf("转移金额必须大于 0")
	}
	return user.TransferAffQuotaToQuota(quota)
}
...
func (user *User) Insert(inviterId int) error {
	...
	grantRegistrationAffiliateRewards(user.Id, inviterId)
	return nil
}
